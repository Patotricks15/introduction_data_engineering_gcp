...
export TF_VAR_create_stream=false
...
echo "Creating PostgreSQL and BigQuery resources..."
terraform -chdir="$TERRAFORM_DIR" apply -auto-approve -input=false

export PGHOST="$(terraform -chdir="$TERRAFORM_DIR" output -raw database_host)"
export PGDATABASE="$(terraform -chdir="$TERRAFORM_DIR" output -raw database_name)"
export PGPASSWORD="$(terraform -chdir="$TERRAFORM_DIR" output -raw postgres_password)"
...
echo "Initializing PostgreSQL for logical replication..."
"$VENV_DIR/bin/python" "$ROOT_DIR/src/database.py" initialize

echo "Creating and starting the Datastream stream..."
export TF_VAR_create_stream=true
terraform -chdir="$TERRAFORM_DIR" apply -auto-approve -input=false

echo "Committing a change for CDC..."
"$VENV_DIR/bin/python" "$ROOT_DIR/src/database.py" insert-change

echo "Waiting for the change in BigQuery..."
"$VENV_DIR/bin/python" "$ROOT_DIR/src/verify_replication.py" \
  --project-id "$PROJECT_ID" \
  --dataset-id "$DATASET_ID" \
  --order-id 1004

echo "Datastream replication completed successfully."
