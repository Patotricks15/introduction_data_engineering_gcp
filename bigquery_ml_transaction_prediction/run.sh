...
if [[ "$LOCATION" != "US" ]]; then
  echo "GCP_LOCATION must be US because the public Google Analytics dataset is in US." >&2
  exit 1
fi
...
echo "Creating the BigQuery ML dataset..."
terraform -chdir="$TERRAFORM_DIR" apply -auto-approve -input=false

DATASET_ID="$(terraform -chdir="$TERRAFORM_DIR" output -raw dataset_id)"
MODEL_ID="$(terraform -chdir="$TERRAFORM_DIR" output -raw model_id)"
PREDICTION_TABLE_ID="$(terraform -chdir="$TERRAFORM_DIR" output -raw prediction_table_id)"

echo "Preparing the Python environment..."
python3 -m venv "$VENV_DIR"
"$VENV_DIR/bin/python" -m pip install --quiet --requirement "$ROOT_DIR/requirements.txt"

echo "Training, evaluating, and scoring the BigQuery ML model..."
"$VENV_DIR/bin/python" "$ROOT_DIR/src/train_and_predict.py" \
  --project-id "$PROJECT_ID" \
  --location "$LOCATION" \
  --dataset-id "$DATASET_ID" \
  --model-id "$MODEL_ID" \
  --prediction-table-id "$PREDICTION_TABLE_ID" \
  --sql-dir "$ROOT_DIR/sql"

echo "BigQuery ML pipeline completed successfully."